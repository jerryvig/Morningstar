...
CREATE TEMPORARY TABLE ordered_data ( domain TEXT, year_month TEXT, unique_visitors INTEGER );

INSERT INTO ordered_data SELECT domain, year_month, unique_visitors FROM compete_data ORDER BY domain ASC, year_month ASC;

CREATE TEMPORARY TABLE visitor_growth ( domain TEXT, year_month TEXT, unique_visitors INTEGER, visitor_growth REAL );
...
CREATE TEMPORARY TABLE sharpe_growth ( domain TEXT, max_unique_visitors, sharpe_growth REAL );

INSERT INTO sharpe_growth SELECT domain, MAX(unique_visitors), AVG(visitor_growth)/STDEV(visitor_growth) FROM visitor_growth GROUP BY domain;

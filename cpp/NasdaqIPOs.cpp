...
int main() {
  ...
  tabStrings.push_back( "upcoming" );
  tabStrings.push_back( "filings" );

  vector<string> monthStrings;
  monthStrings.push_back("2011-01");
  monthStrings.push_back("2011-02");
  monthStrings.push_back("2011-03");
  monthStrings.push_back("2011-04");
  monthStrings.push_back("2011-05");
  monthStrings.push_back("2011-06");
  monthStrings.push_back("2011-07");
  monthStrings.push_back("2011-08");
  monthStrings.push_back("2011-09");
  monthStrings.push_back("2011-10");
  ...
  for ( vector<string>::iterator iter = tabStrings.begin(); iter != tabStrings.end(); iter++ ) {
	...
	cout << "Scraping " << *iter << " for month = " << *monthIter << endl;
        try {
          ...
          myRequest.setOpt(curlpp::options::Url((std::string( "http://www.nasdaq.com/markets/ipos/activity.aspx?tab="+*iter+"&month="+*monthIter))));
          ostringstream os;
          os << myRequest;
          ...
          if ( *iter == "pricings" || *iter == "upcoming" ) {
            string type = "";
            if ( *iter == "pricings" ) {
	      type = "PRICING";
            }
            else if ( *iter == "upcoming" ) {
              type = "UPCOMING";
            }
